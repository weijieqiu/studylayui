...
public class ExcelReader {
    ...
    public static List<ExcelDataVO> readExcel(String fileName) {
        ...
        try {
            ...
            logger.debug("解析Excel失败，文件名：" + fileName + " 错误信息：" + e.getMessage());
            return null;
        } finally {
            try {
                if (null != workbook) {
    ...
    public static List<ExcelDataVO> readExcel(MultipartFile file) {
        ...
        try {
            ...
            if (fileName == null || fileName.isEmpty() || fileName.lastIndexOf(".") < 0) {
                logger.debug("解析Excel失败，因为获取到的Excel文件名非法！");
                return null;
            }
            String fileType = fileName.substring(fileName.lastIndexOf(".") + 1, fileName.length());
            ...
            logger.debug("解析Excel失败，文件名：" + file.getOriginalFilename() + " 错误信息：" + e.getMessage());
            return null;
        } finally {
            try {
                if (null != workbook) {
       ...
       List<ExcelDataVO> resultDataList = new ArrayList<>();
        ...
        for (int sheetNum = 0; sheetNum < workbook.getNumberOfSheets(); sheetNum++) {
            ...
            for (int rowNum = rowStart; rowNum < rowEnd; rowNum++) {
                ...
                if (null == resultData) {
                    logger.debug("第 " + row.getRowNum() + "行数据不合法，已忽略！");
                    continue;
                }
